...
import o_voxel
import cumesh
from o_voxel.rasterize import rasterize_mesh_attributes, rasterize_mesh_attributes_drtk
import flex_gemm
import pyuvula
...
class Trellis2TexturingPipeline(Pipeline):
    ...
    @classmethod
    def from_pretrained(cls, path: str, model_revisions: dict[str, str], config_file: str = "texturing_pipeline.json") -> "Trellis2TexturingPipeline":
        """
    ...
    def sample_tex_slat(
        ...
        noise = shape_slat.replace(
            feats=torch.randn(shape_slat.coords.shape[0], in_channels - shape_slat.feats.shape[1], generator=generator).to(self.device)
        )
    ...
    def postprocess_mesh(
        ...
        if hasattr(mesh, 'visual') and hasattr(mesh.visual, 'uv') and mesh.visual.uv is not None:
            ...
            if uv_unwrapping_backend == "xatlas":
                ...
                vertices_torch, faces_torch, uvs_torch, vmap = _cumesh.uv_unwrap(
                    ...
                    # xatlas_pack_charts_kwargs={"padding": 5},
                    return_vmaps=True
                )
                ...
                uvs, vertices, faces, _, _, vmap = pyuvula.unwrap(np.array(mesh.vertices), np.array(mesh.faces))
                normals = normals[vmap]  # remap original smooth normals, like xatlas does
                vertices_torch = torch.from_numpy(vertices).float().cuda()
                faces_torch = torch.from_numpy(faces).int().cuda()
                uvs_torch = torch.from_numpy(uvs).float().cuda()
            else:
        ...
        if rast_backend == "kaolin":
            logger.info("Using kaolin rasterizer.")
            pos, mask = rasterize_mesh_attributes(
                uvs_torch, faces_torch, vertices_torch, texture_size
            )
        elif rast_backend == "drtk":
            logger.info("Using drtk rasterizer.")
            pos, mask = rasterize_mesh_attributes_drtk(
                uvs_torch, faces_torch, vertices_torch, texture_size
            )
        else:
        ...
        # Enhanced inpainting for any remaining gaps
        inpaint_mask = (~mask).astype(np.uint8)

        # Inpaint with larger radius for better coverage
        base_color = cv2.inpaint(base_color, inpaint_mask, 3, cv2.INPAINT_TELEA)            # 3
        metallic = cv2.inpaint(metallic, inpaint_mask, 1, cv2.INPAINT_TELEA)[..., None]     # 1
        roughness = cv2.inpaint(roughness, inpaint_mask, 1, cv2.INPAINT_TELEA)[..., None]   # 1
        alpha = cv2.inpaint(alpha, inpaint_mask, 1, cv2.INPAINT_TELEA)[..., None]           # 1
    ...
    def run(
        ...
        out_mesh = self.postprocess_mesh(mesh, pbr_voxel, resolution, texture_size, uv_unwrapping_backend, rast_backend)
        return out_mesh