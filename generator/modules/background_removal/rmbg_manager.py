...
from generator.config import bg_remover_settings
...
class BackgroundRemovalService(ABC):
    def __init__(self, model_versions: dict) -> None:
        ...
        """
        self.settings = bg_remover_settings
    ...
    async def startup(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error loading background removal model model: {e}")
            raise RuntimeError(f"Error loading background removal model model: {e}")
    ...
    def remove_background(self, image: Image.Image | Iterable[Image.Image]) -> Image.Image | Iterable[Image.Image]:
        ...
        images_without_background = list(to_pil_image(o[:3]) for o in outputs) if isinstance(image, Iterable) else to_pil_image(outputs[0][:3])
