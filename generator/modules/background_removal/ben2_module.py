...
class BEN2BackgroundRemovalService(BackgroundRemovalService):
    ...
    def _load_model(self, model_id: str = "PramaLLC/BEN2") -> BEN_Base:
        """
        Load the BEN2 background removal model.
        """
        revision = self.model_versions.get(model_id)
        model = BEN_Base.from_pretrained(model_id, revision=revision)
        logger.success(f"{model_id} model loaded.")
