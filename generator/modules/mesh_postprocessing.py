...
def to_glb(
    ...
    assert isinstance(aabb,
                      torch.Tensor), f"aabb must be a list, tuple, np.ndarray, or torch.Tensor, but got {type(aabb)}"
    ...
    if rast_backend == "kaolin":
        pos, mask = rasterize_mesh_attributes(
            out_uvs, out_faces, out_vertices, texture_size
        )
