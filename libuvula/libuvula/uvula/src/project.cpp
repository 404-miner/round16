...
Clipper2Lib::Path64 toPath(const RangeOrInitList& polygon)
{
    Clipper2Lib::Path64 path;
    path.reserve(std::size(polygon));
    for (const Point2F& point : polygon)
    {
        path.push_back(Clipper2Lib::Point64(std::llround(point.x * CLIPPER_PRECISION), std::llround(point.y * CLIPPER_PRECISION)));
    }
    return path;
}

Clipper2Lib::Paths64 intersect(const Clipper2Lib::Path64& polygon1, const Clipper2Lib::Path64& polygon2)
{
    return Clipper2Lib::Intersect(Clipper2Lib::Paths64{ polygon1 }, Clipper2Lib::Paths64{ polygon2 }, Clipper2Lib::FillRule::EvenOdd);
}

Clipper2Lib::Paths64 unionPaths(const Clipper2Lib::Paths64& paths)
{
    return Clipper2Lib::Union(paths, Clipper2Lib::FillRule::EvenOdd);
}

std::vector<Polygon> toPolygons(const Clipper2Lib::Paths64& paths)
{
...
{
    std::vector<Polygon> result;
    const Clipper2Lib::Path64 stroke_polygon_path = toPath(stroke_polygon);
    ...
    while (! candidate_faces.empty())
    {
        ...
        const Clipper2Lib::Path64 projected_face_triangle_path
    ...
    Clipper2Lib::Paths64 uv_areas_path;
